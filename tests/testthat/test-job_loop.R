...
test_that("job_loop", {
    ...
    expect_error(run_test(email = 'hello', loops = list('a' = letters)),
        "'email' should be one of")
    expect_error(run_test(logdir = '/logs', loops = list('a' = letters)),
        'relative path')
    expect_error(run_test(cores = 0.5, loops = list('a' = letters)),
        'should be at least 1')
    expect_equal(grep('pe local', run_test(cores = 1,
        loops = list('a' = letters))), integer(0))
    expect_equal(grepl('#\\$ -pe local 10', run_test(cores = 10,
        loops = list('a' = letters))), TRUE)
    expect_equal(grep('shared', run_test(queue = 'shared',
        loops = list('a' = letters))), integer(0))
